...
const Su = ({ selectedNeighborhood }) => {
  const [neighborhoods] = useState(
    NEIGHBORHOOD_DATA.map((n) => ({
      name: n.name,
      value: `${(n.water * 1000).toLocaleString("tr-TR")} m³`,
      change: `${((n.water - 1.0) / 1.0 * 100).toFixed(1)}%`,
      isIncrease: n.water > 1.0,
    }))
  );
    ...
    <div className="pt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-12">
      <div className="animate-fade-in">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Mahalle bazında ortalama su tüketim değerleri</p>
        </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-2xl font-bold text-blue-500 mt-2">{neighborhood.value}</p>
                  </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-gray-600">Toplam Su Tüketimi</span>
                <Droplets className="w-5 h-5 text-blue-500" />
              </div>
              <p className="text-3xl font-bold text-gray-900">685K m³</p>
            </div>
          </div>
        </div>
      </div>
