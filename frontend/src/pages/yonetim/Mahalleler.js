...
/* =========================================================================
   1. YARDIMCI BİLEŞEN: LİSTE SATIRI (Aynı)
   ========================================================================= */
...
/* =========================================================================
   2. YARDIMCI BİLEŞEN: MODAL İÇİNDEKİ KAYNAK KARTI
   ========================================================================= */
const KaynakKarti = ({ title, icon: Icon, color, isSelected, onClick, onAriza, loading }) => {
    const colors = { 
    ...
    return (
        ...
        >
            ...
            {/* Arıza Butonu Sadece Seçiliyse Görünür */}
            {isSelected && (
                <button 
                    onClick={(e) => {
...
/* =========================================================================
   3. ANA MODAL BİLEŞENİ (DİNAMİK GRAFİK VE KAYAN PENCERE EKLENDİ)
   ========================================================================= */
const MahalleDetayModal = ({ mahalleName, onClose, onIncidentCreated }) => {
    const [loading, setLoading] = useState(false);
    const [selectedSource, setSelectedSource] = useState('Elektrik'); // Varsayılan: Elektrik
    const [graphData, setGraphData] = useState([]); // Grafik verileri
    const [lastUpdate, setLastUpdate] = useState(new Date());

    // --- (GÖREV 2) 15 SANİYEDE BİR VERİ ÇEKME ---
    const fetchMahalleData = useCallback(async () => {
        try {
            // NOT: Backend'de '/stats/mahalle/:mahalleAdi' gibi bir endpoint olduğunu varsayıyorum.
            // Yoksa tüm listeyi çekip filtrelemen gerekebilir ama performans için tek mahalle endpointi iyidir.
            // Şimdilik simülasyon yapıyorum çünkü backend endpointini bilmiyorum.
            
            // Simüle edilmiş veri (Burayı backend endpointinle değiştirebilirsin)
            // Gerçekte: const res = await api.get(`/stats/mahalle/${mahalleName}`);
            
            // Rastgele veri üretimi (Grafik kaymasını test etmek için)
    ...
    useEffect(() => {
        fetchMahalleData(); // İlk açılışta çek
        const interval = setInterval(fetchMahalleData, 15000); // 15 sn'de bir yenile
        return () => clearInterval(interval); // Kapanınca temizle
    }, [fetchMahalleData]);


    // --- (GÖREV 2) GRAFİK SAATLERİNİ KAYDIRMA ---
    // Şu anki saatten geriye doğru 24 saati hesaplar
    const timeLabels = useMemo(() => {
        ...
        return labels;
    }, [lastUpdate]); // Her güncellemede saatler de kayabilir (dakika bazlı yaparsan)


    // Arıza Kayıt Fonksiyonu
    const handleAriza = async () => {
      if(!window.confirm(`${mahalleName} için ${selectedSource} arıza kaydı oluşturulsun mu?`)) return;
      ...
      try {
          const newIncident = {
              ...
              Aciklama: `${mahalleName} mahallesinde ${selectedSource} arızası (Grafik ekranından manuel)`,
          };

          await api.post('/incidents/instant', newIncident); 
          alert(`${mahalleName} için ${selectedSource} arızası kaydedildi!`);
          onIncidentCreated(); 
          ...
          <div className="bg-gray-900 px-8 py-6 flex justify-between items-center text-white shrink-0">
            <div>
              <h2 className="text-3xl font-bold flex items-center gap-3">
                <MapPin className="text-emerald-400" /> {mahalleName}
              </h2>
              <p className="text-gray-400 text-sm mt-1 flex items-center gap-2">
                <Activity size={14}/> Canlı İzleme Modu • Son Güncelleme: {lastUpdate.toLocaleTimeString()}
              </p>
            </div>
          ...
          <div className="flex flex-col md:flex-row h-full overflow-hidden">
             ...
             <div className="w-full md:w-1/3 bg-gray-50 p-6 border-r border-gray-200 overflow-y-auto">
                ...
                <div className="grid gap-4">
                    <KaynakKarti 
                        title="Elektrik" icon={Zap} color="yellow" 
                        isSelected={selectedSource === 'Elektrik'} 
                        onClick={() => setSelectedSource('Elektrik')} 
                        onAriza={handleAriza} loading={loading}
                    />
                    <KaynakKarti 
                        title="Su" icon={Droplets} color="blue" 
                        isSelected={selectedSource === 'Su'} 
                        onClick={() => setSelectedSource('Su')} 
                        onAriza={handleAriza} loading={loading}
                    />
                    <KaynakKarti 
                        title="Doğalgaz" icon={Flame} color="orange" 
                        isSelected={selectedSource === 'Doğalgaz'} 
                        onClick={() => setSelectedSource('Doğalgaz')} 
                        onAriza={handleAriza} loading={loading}
                    />
                </div>
             </div>
             ...
             <div className="w-full md:w-2/3 p-8 flex flex-col bg-white">
                <div className="flex justify-between items-center mb-8">
                    <h3 className="font-bold text-gray-800 text-xl flex items-center gap-2">
                        ...
                        {selectedSource} Tüketim Grafiği (Son 24 Saat)
                    </h3>
                ...
                <div className="flex-1 flex items-end justify-between gap-1 relative border-b border-l border-gray-200 p-4 min-h-[300px]">
                   {/* Arkaplan Çizgileri */}
                   <div className="absolute inset-0 pointer-events-none flex flex-col justify-between text-xs text-gray-300 px-2 pb-8">
                   ...
                   {graphData.map((val, i) => (
                      <div key={i} className="flex-1 flex flex-col justify-end items-center group h-full relative z-10">
                        ...
                        <div className="absolute -top-10 bg-gray-900 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">
                            {timeLabels[i]} : {val} birim
                        </div>
                        {/* Bar */}
                        <div 
                            className={`w-full rounded-t-sm transition-all duration-500 ${
                                selectedSource === 'Elektrik' ? 'bg-yellow-400 hover:bg-yellow-500' : 
                                selectedSource === 'Su' ? 'bg-blue-400 hover:bg-blue-500' : 
                ...
                {/* SAAT ETİKETLERİ (Kayan Yazı) */}
                <div className="flex justify-between text-[10px] text-gray-400 mt-2 font-mono">
                   {/* Sadece belirli saatleri göster ki sıkışmasın */}
                   <span>{timeLabels[0]}</span>
                   <span>{timeLabels[6]}</span>
                   <span>{timeLabels[12]}</span>
                   <span>{timeLabels[18]}</span>
                   <span>{timeLabels[23]}</span>
                </div>

                <div className="mt-6 p-4 bg-gray-50 rounded-xl text-sm text-gray-500 border border-gray-100">
                    <p>ℹ️ Grafik verileri her 15 saniyede bir güncellenir. Görüntülenen veriler <strong>{timeLabels[0]}</strong> ile <strong>{timeLabels[23]}</strong> saatleri arasını kapsar.</p>
                </div>
             </div>
...
/* =========================================================================
   4. ANA SAYFA BİLEŞENİ (BUTON İSMİ DÜZELTİLDİ)
   ========================================================================= */
const Mahalleler = () => {
    const [mahalleler, setMahalleler] = useState([]);
    const [loading, setLoading] = useState(true);
    const [modalMahalleName, setModalMahalleName] = useState(null); // Sadece isim tutuyoruz
    ...
    useEffect(() => {
        fetchData();
        // Ana liste de 30 saniyede bir güncellensin
    ...
    if (loading && mahalleler.length === 0) {
    ...
    return (
        <div className="container mx-auto px-4 py-8 pt-[150px] min-h-screen relative z-0">
            ...
            <div className="bg-emerald-100/50 py-6 px-8 rounded-3xl mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4 shadow-sm border border-emerald-100 relative z-10">
                ...
                <div className="relative w-full max-w-sm ml-auto mt-4 md:mt-0">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-400 transition-all text-gray-800"
                    />
                </div>
            </div>
                ...
                {filteredMahalleler.length > 0 ? (
                    filteredMahalleler.map((mahalle, index) => (
                        <div key={index} className="bg-white p-6 rounded-3xl shadow-lg hover:shadow-2xl transition-all border border-gray-100 flex flex-col group">
                            ...
                            {/* --- (GÖREV 2) BUTON İSMİ DEĞİŞTİ --- */}
                            <button 
                                onClick={() => setModalMahalleName(mahalle.mahalle)}
                                className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-xl font-bold transition-all shadow-md group-hover:scale-[1.02] active:scale-95 cursor-pointer"
                            >
                    ...
                    <div className="col-span-full bg-white p-8 rounded-xl text-center text-gray-600">
                        Aradığınız kriterlere uygun mahalle bulunamadı.
                    </div>
                )}
            </div>
            ...
            {modalMahalleName && (
                <MahalleDetayModal 
                    mahalleName={modalMahalleName} 
                    onClose={() => setModalMahalleName(null)}
